...
observer = Observer(4, pi / 2, 0, 0.999, 0.0, 0.0, 1.0)


for phi in np.linspace(0, 2 * pi, 10, endpoint=False):
    ray = observer.fido_ray(sin(phi), 1, cos(phi))
    data = ([], [], [])
    for i in range(1000):
        try:
            ray.euler_step(0.0025)
        except:
            break
        _r = ray.r
        _theta = ray.theta
    ...
    ax.plot(*data, label="parametric curve")
